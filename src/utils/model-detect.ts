...
export async function detectImage(imageUrl: string, session: { net: InferenceSession; nms: InferenceSession } | null) {
  const MODEL_SHAPES = [1, 3, 800, 800];
  const IOU_THRESHOLD = 0.45;
  const SCORE_THRESHOLD = 0.7; // minimum confidence values to be returned

  const [modelWidth, modelHeight] = MODEL_SHAPES.slice(2);
  const input = await getImageTensorFromPath(imageUrl, modelWidth, modelHeight);

  const tensor = new Tensor("float32", input.data, MODEL_SHAPES);
  const config = new Tensor("float32", new Float32Array([100, IOU_THRESHOLD, SCORE_THRESHOLD]));
